...
export const GlobalSearchModal: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Search Header Input */}
        <div className="flex items-center px-4 py-3.5 border-b border-slate-200 dark:border-slate-700/50 gap-3">
          ...
          <button 
            ...
            onClick={() => setGlobalSearchOpen(false)} 
            className="text-xs px-2 py-1 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 border border-slate-200 dark:border-slate-700"
          >
            Esc
          </button>
        </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-b border-slate-200 dark:border-slate-700/40 overflow-x-auto text-xs scrollbar-none">
          {[
            { id: 'all', label: 'All' },
            ...
            results.map((item, idx) => (
              <div
                ...
                className={`flex items-start justify-between p-3 rounded-xl cursor-pointer transition-colors ${
                  selectedIndex === idx
                    ? 'bg-indigo-50 dark:bg-indigo-600/20 border border-indigo-200 dark:border-indigo-500/40 text-indigo-950 dark:text-white'
              ...
              >
                <div className="flex items-start gap-3 min-w-0 pr-2">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-sm truncate text-slate-900 dark:text-slate-100">{item.title}</span>
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-mono shrink-0">
                        {item.category}
                      </span>
                    </div>
                ...
                <ArrowRight className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0 mt-2" />
              </div>
            ))
          ) : (
            <div className="py-12 text-center text-slate-500 dark:text-slate-400">
              <p className="text-sm">No results found for "{query}" in this category.</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Try searching for "PDF", "JSON", "Python", "Battery", or "CSS".</p>
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-2 bg-slate-50 dark:bg-slate-950/60 border-t border-slate-200 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between">
          ...
          <span className="text-indigo-600 dark:text-indigo-400 font-medium">{results.length} results indexed</span>
        </div>
      </div>
    </div>
  );
