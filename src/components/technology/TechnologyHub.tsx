    ...
    <div id="tech-hub-container" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {selectedArticle ? (
        <div id="tech-article-view" className="max-w-4xl mx-auto space-y-6">
          ...
          <article className="p-6 sm:p-10 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
            <div className="space-y-3 pb-6 border-b border-slate-200 dark:border-slate-800">
              ...
              <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-slate-100">{selectedArticle.title}</h1>
            ...
            <div className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed space-y-4 whitespace-pre-line font-sans">
        ...
        <div id="tech-category-view" className="space-y-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-slate-900 dark:text-slate-100">{currentCategory.title}</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{currentCategory.description}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {currentCategory.articles.map(article => (
              <div
                ...
                className="group p-6 rounded-2xl bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 cursor-pointer transition-all flex flex-col justify-between space-y-4 shadow-xs hover:shadow-xl hover:shadow-indigo-950/10"
              >
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-center gap-2">
            <div className="flex items-center p-1.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs">
              <button
                onClick={() => setActiveTab('categories')}
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  activeTab === 'categories' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
                Core Tech Domains
              </button>
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                  activeTab === 'troubleshoot' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
          ...
          {activeTab === 'categories' ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {TECH_CATEGORIES.map(cat => (
                <div
                  ...
                  className="group p-6 rounded-2xl bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 cursor-pointer transition-all duration-200 space-y-4 shadow-xs hover:shadow-xl hover:shadow-indigo-950/10"
                >
                  <div className="flex items-center justify-between">
                    <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 group-hover:border-indigo-500/40 transition-colors">
                  ...
                  <div className="flex items-center justify-between text-xs font-semibold text-indigo-600 dark:text-indigo-400 pt-2">
            ...
            <div className="space-y-4 max-w-4xl mx-auto">
              <div className="p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/30 flex items-center gap-3 text-xs text-indigo-700 dark:text-indigo-300">
                <Wrench className="w-5 h-5 shrink-0 text-indigo-600 dark:text-indigo-400" />
                <span>
