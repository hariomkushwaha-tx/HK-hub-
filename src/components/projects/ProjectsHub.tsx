    ...
    <div id="projects-hub-container" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-indigo-500 shadow-xs placeholder-slate-400"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProjects.map(project => (
          <div
            key={project.id}
            className="p-6 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 flex flex-col justify-between space-y-4 hover:border-indigo-400 dark:hover:border-indigo-500/40 transition-colors shadow-xs"
          >
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-700 dark:text-indigo-400 font-mono px-2.5 py-0.5 rounded-md bg-indigo-50 dark:bg-indigo-500/15 border border-indigo-200 dark:border-indigo-500/25">
                  {project.category}
                </span>
                <button
                  onClick={() => toggleLikeProject(project.id)}
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold border border-slate-200 dark:border-slate-700 transition-colors"
                >
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                {project.technologies.map((t, idx) => (
                  <span key={idx} className="text-[10px] px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800/80 text-slate-600 dark:text-slate-300 font-mono border border-slate-200 dark:border-slate-700/50">
                    {t}
                  </span>
                ))}
              </div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs">
              <span className="text-slate-500 dark:text-slate-400">
                By <strong className="text-slate-800 dark:text-slate-300">@{project.authorUsername}</strong>
              </span>
              ...
              <div className="flex items-center gap-2">
                ...
                {project.liveDemoUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-2.5 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-600/20 hover:bg-indigo-100 dark:hover:bg-indigo-600/30 text-indigo-600 dark:text-indigo-300 font-semibold flex items-center gap-1 border border-indigo-200 dark:border-indigo-500/30 transition-colors"
                  >
                    <span>Demo</span>
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <div>
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Project Title *</label>
                <input
                  type="text"
                  required
                  value={newTitle}
                  ...
                  className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-indigo-500 shadow-xs"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 outline-none shadow-xs"
                  >
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Your Name / Handle</label>
                  <input
                    type="text"
                    value={newAuthor}
              ...
              <div>
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Tech Stack (comma separated)</label>
                <input
                  type="text"
                  value={newTech}
              ...
              <div>
                <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Short Description & Highlights *</label>
                <textarea
                  rows={3}
                  required
                  ...
                  className="w-full p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 outline-none resize-none shadow-xs"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">GitHub Repository URL</label>
                  <input
                    type="url"
                    value={newGithub}
                    onChange={e => setNewGithub(e.target.value)}
                    placeholder="https://github.com/..."
                    className="w-full p-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 outline-none shadow-xs"
                  />
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 block">Live Demo URL</label>
                  <input
                    type="url"
                    value={newLive}
