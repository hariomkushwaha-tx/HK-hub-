    ...
    <div id="guides-hub-container" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      {activeGuide ? (
        ...
        <div id="active-guide-reader" className="max-w-4xl mx-auto space-y-6">
          ...
          <article className="p-6 sm:p-10 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-8 shadow-sm">
            {/* Header Meta */}
            <div className="space-y-4 pb-6 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">{activeGuide.difficulty}</span>
                </div>
            ...
            <div className="space-y-6">
              <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">Step-by-Step Instructions</h3>
              <div className="space-y-4">
                {activeGuide.steps.map((step, idx) => (
                  <div
                    key={idx}
                    className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 space-y-2"
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="w-6 h-6 rounded-full bg-indigo-500/10 dark:bg-indigo-600/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center text-xs font-bold font-mono">
                        {idx + 1}
                      </span>
              ...
              <div className="p-5 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 space-y-3">
                ...
                <div className="space-y-1.5 pl-6">
                  {activeGuide.warnings.map((w, i) => (
                    <div key={i} className="text-xs text-amber-800 dark:text-amber-200/90 flex items-start gap-2">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-indigo-500 shadow-xs transition-colors"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredGuides.map(guide => (
              <div
                ...
                className="group p-6 rounded-2xl bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 cursor-pointer transition-all duration-200 flex flex-col justify-between space-y-4 shadow-xs hover:shadow-xl hover:shadow-indigo-950/10"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-400 font-mono text-[10px] px-2 py-0.5 rounded bg-indigo-50 dark:bg-slate-800 border border-indigo-100 dark:border-slate-700">
                      {guide.category}
                    </span>
                ...
                <div className="pt-3 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs font-semibold text-indigo-600 dark:text-indigo-400">
