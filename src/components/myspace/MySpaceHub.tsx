  ...
  return (
    <div id="myspace-view" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 space-y-10">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-tr from-indigo-600 via-blue-600 to-cyan-500 flex items-center justify-center text-white text-2xl font-black shadow-md shadow-indigo-500/20">
              {displayName.charAt(0).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100">{displayName}</h1>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30">
                  {userProfile?.role || 'Student'}
                </span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400">@{userProfile?.username || 'tech_explorer'} • HK VELORA Member</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold transition-colors"
            >
        ...
        {isEditing ? (
          <form onSubmit={handleSave} className="p-4 sm:p-6 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs text-slate-900 dark:text-slate-100 outline-none focus:border-indigo-500 shadow-xs"
                />
              </div>
              <div>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Bio & Learning Focus</label>
              <textarea
                rows={2}
                value={bio}
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {(userProfile?.skills || []).map((skill, idx) => (
                <span
                  key={idx}
                  className="text-xs px-3 py-1 rounded-lg bg-indigo-50 dark:bg-indigo-600/15 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/25 font-mono font-medium"
                >
                  {skill}
                </span>
              ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Saved Utilities', val: savedTools.length, icon: <Wrench className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> },
          { label: 'Saved E-Books', val: savedBooks.length, icon: <BookMarked className="w-5 h-5 text-pink-600 dark:text-pink-400" /> },
          { label: 'Bookmarked Guides', val: savedGuides.length, icon: <BookOpen className="w-5 h-5 text-blue-600 dark:text-blue-400" /> },
          { label: 'Showcased Projects', val: userProjects.length, icon: <FolderGit2 className="w-5 h-5 text-emerald-600 dark:text-emerald-400" /> },
        ].map((s, idx) => (
          <div key={idx} className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 flex items-center gap-3 shadow-xs">
            <div className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-800">{s.icon}</div>
            <div>
              <p className="text-lg font-black text-slate-900 dark:text-slate-100">{s.val}</p>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Personal My Library ({savedBooks.length})</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">Continue reading, review bookmarks, and access unlocked textbooks</p>
            </div>
          </div>
          <button
            onClick={() => setActiveTab('ebooks')}
            className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 self-start sm:self-auto"
          >
            Explore E-Books Hub →
          </button>
        </div>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl font-semibold whitespace-nowrap transition-all ${
                libraryFilter === f.id
                  ...
                  : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 shadow-xs'
              }`}
            >
              {f.label}
            </button>
        ...
        {filteredLibraryBooks.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredLibraryBooks.map(book => {
              ...
              return (
                <div
                  key={book.id}
                  className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500/40 transition-all flex flex-col justify-between space-y-4 shadow-xs relative overflow-hidden"
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-mono uppercase tracking-wider text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded bg-indigo-50 dark:bg-slate-800 border border-indigo-100 dark:border-slate-700">
                        {book.category}
                      </span>
                    ...
                    <h3 className="font-bold text-sm text-slate-900 dark:text-slate-100 mt-2 line-clamp-1">{book.title}</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400">By {book.author}</p>
                    <p className="text-xs text-slate-600 dark:text-slate-300 line-clamp-2 mt-2 leading-relaxed">{book.description}</p>
                  </div>
                  ...
                  {prog && (
                    <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-1.5">
                      <div className="flex items-center justify-between text-[10px]">
                        <span className="text-indigo-600 dark:text-indigo-400 font-semibold truncate max-w-[170px]">
                          {prog.currentChapterTitle}
                        </span>
                        <span className="font-mono text-slate-700 dark:text-slate-300 font-bold">{prog.percentage}%</span>
                      </div>
          ...
          <div className="p-8 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-dashed border-slate-200 dark:border-slate-800 text-center space-y-2">
            <p className="text-sm text-slate-500 dark:text-slate-400">No books found matching this filter.</p>
            <button
              onClick={() => {
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Saved Tools ({savedTools.length})</h2>
          </div>
          <button
        ...
        {savedTools.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {savedTools.map(t => (
              <div
                ...
                className="p-5 rounded-2xl bg-white dark:bg-slate-900/80 hover:bg-slate-50 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-400 dark:hover:border-indigo-500/40 cursor-pointer transition-all flex flex-col justify-between space-y-3 shadow-xs"
              >
                <div>
                  <span className="text-[10px] font-mono uppercase tracking-wider text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded bg-indigo-50 dark:bg-slate-800 border border-indigo-100 dark:border-slate-700">
                    {t.category}
                  </span>
                  <h3 className="font-bold text-sm text-slate-900 dark:text-slate-100 mt-2">{t.name}</h3>
          ...
          <div className="p-8 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-dashed border-slate-200 dark:border-slate-800 text-center space-y-2">
            <p className="text-sm text-slate-500 dark:text-slate-400">You haven't bookmarked any tools yet.</p>
            <button
              onClick={() => setActiveTab('tools')}
              className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs"
            >
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100">Saved Guides & Tutorials ({savedGuides.length})</h2>
          </div>
          <button
        ...
        {savedGuides.length > 0 ? (
            ...
            {savedGuides.map(g => (
              ...
              >
                <div>
                  ...
                  <h3 className="font-bold text-sm text-slate-900 dark:text-slate-100 mt-2">{g.title}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2 mt-1">{g.intro}</p>
                </div>
                <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 pt-2 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <span className="text-indigo-600 dark:text-indigo-400 font-semibold">Read →</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="p-8 rounded-2xl bg-slate-50 dark:bg-slate-900/40 border border-dashed border-slate-200 dark:border-slate-800 text-center space-y-2">
            <p className="text-sm text-slate-500 dark:text-slate-400">No guides saved yet.</p>
            <button
              onClick={() => setActiveTab('guides')}
              className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-xs"
            >
